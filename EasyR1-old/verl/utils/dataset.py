...
def process_image(
    ...
    # Ensure minimum dimensions for Qwen2.5-VL processor (factor constraint of 28)
    if image.width < 28 or image.height < 28:
        # Calculate scale factor to make both dimensions at least 28
        scale_factor = max(28 / image.width, 28 / image.height)
        width, height = int(image.width * scale_factor), int(image.height * scale_factor)
        image = image.resize((width, height))

    # Optimized size limiting to prevent MRoPE IndexError
    # vLLM processes images into patches, so we need controlled dimensions
    max_dimension = 224  # Force smaller images to prevent large grids
    if image.width > max_dimension or image.height > max_dimension:
        # Calculate scale factor to fit within max_dimension
        scale_factor = min(max_dimension / image.width, max_dimension / image.height)
        width, height = int(image.width * scale_factor), int(image.height * scale_factor)
        image = image.resize((width, height), Image.Resampling.LANCZOS)  # High-quality resampling
        # Resize operation completed silently
