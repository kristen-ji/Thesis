...
# Check if flash attention should be disabled via environment variable
_flash_attention_disabled = os.getenv("FLASH_ATTENTION_FORCE_DISABLE", "0") == "1"
_flash_attn_available = False
_flash_supports_window_size = False
_flash_supports_deterministic = False
_flash_deterministic_enabled = False
_flash_use_top_left_mask = True

if not _flash_attention_disabled:
    try:
        ...
        # Flash attention import failed (e.g., ABI incompatibility), fallback to standard attention
        _flash_attn_available = False
        import warnings
        warnings.warn("Flash attention unavailable (ABI mismatch). Using standard attention.")
...
def _custom_flash_attention_forward(
    ...
    **kwargs,
    ...
    if _flash_attn_available and position_ids is not None and query_length != 1 and not (torch.diff(position_ids, dim=-1) >= 0).all():
