...
def get_rope_index(
    ...
    if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
        ...
        # ROOT CAUSE FIX: Ensure we don't process more images than available
        actual_image_count = len(image_grid_thw) if image_grid_thw is not None else 0
        actual_video_count = len(video_grid_thw) if video_grid_thw is not None else 0
        
        if actual_image_count > 0 and image_nums > actual_image_count:
            remain_images = actual_image_count
            image_nums = actual_image_count
            
        if actual_video_count > 0 and video_nums > actual_video_count:
            remain_videos = actual_video_count
            video_nums = actual_video_count
        ...
        for _ in range(image_nums + video_nums):
            ...
            if ed_image < ed_video:
                # CRITICAL FIX: Ensure image_index doesn't exceed available images
                if image_index >= len(image_grid_thw):
                    image_index = len(image_grid_thw) - 1
                ...
                # CRITICAL FIX: Ensure video_index doesn't exceed available videos
                if video_index >= len(video_grid_thw):
                    video_index = len(video_grid_thw) - 1
